...
import time
import sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "align_utilities"))
from model_classes import BertClassifierPretrained
import utilities as utl
pd.options.mode.chained_assignment = None  # default='warn'

random.seed(42)

# ── BERT embedding generation (used when method == "bert/" and JSON files are missing) ──

_bert_model = None
_bert_tokenizer = None
_MAX_COL_SIZE = 10000   # matches align_phase.py compute_embeddings()
...
def _load_bert():
    ...
    print("Loading sentence-bert (bert-base-uncased) …")
    _bert_model = SentenceTransformer("bert-base-uncased")
    _bert_tokenizer = ""   # not used by sentence-transformers
    print("Sentence-BERT loaded.")

def _get_column_embedding(values):
    """
    Mirrors align_phase.py compute_embeddings() exactly:
      1. unique cell values, capped at _MAX_COL_SIZE
      2. join → clean → re-split into tokens
      3. delegate to utl.EmbedTable (handles sampling + convergence loop)
    """
    column_data = list(set(str(v) for v in values))
    column_data = random.sample(column_data, min(len(column_data), _MAX_COL_SIZE))
    all_text = " ".join(column_data)
    all_text = re.sub(r"\([^)]*\)", "", all_text)
    column_data = list(set(re.sub(r"[^a-z0-9]+", " ", all_text.lower()).split()))
    column_data = [t for t in column_data if t]
    if not column_data:
        return np.random.uniform(-1, 1, 768).astype(np.float32)
    emb = utl.EmbedTable(column_data, embedding_type="sentence_bert",
                         model=_bert_model, tokenizer=_bert_tokenizer)[0]
    if np.isnan(emb).any():
        emb = np.random.uniform(-1, 1, 768).astype(np.float32)
    return emb.astype(np.float32)

def generate_bert_embeddings_for_folder(clusters_dir, output_dir):
    """Generate BERT_table_representations_<cluster>.json for every cluster subfolder."""
    os.makedirs(output_dir, exist_ok=True)
    _load_bert()
    # Only process multi-table clusters (named cluster_XXXX by discover_clusters.py).
    # Singleton folders use the table's own name and are intentionally skipped:
    # a single-table cluster has no cross-table edges, so it contributes nothing
    # to alignment evaluation (P/R/F) and nothing to FD beyond a trivial identity.
    cluster_dirs = sorted(glob.glob(os.path.join(clusters_dir, "cluster_*")))
    print(f"Generating BERT embeddings for {len(cluster_dirs)} multi-table clusters "
          f"(singletons skipped) …")
    for cluster_dir in cluster_dirs:
        cluster_name = os.path.basename(cluster_dir)
        out_file = os.path.join(output_dir, f"BERT_table_representations_{cluster_name}.json")
        if os.path.exists(out_file):
            continue
        cluster_embs = {}
        for csv_path in sorted(glob.glob(os.path.join(cluster_dir, "*.csv"))):
            ...
            try:
                ...
                for col in df.columns:
                    ...
                    if vals:
                        table_embs[col] = _get_column_embedding(vals).tolist()
                if table_embs:
                    cluster_embs[table_name] = table_embs
            except Exception as exc:
                print(f"  Warning {table_name}: {exc}")
        with open(out_file, "w") as f:
            json.dump(cluster_embs, f)
...
method = r"bert/" #change it to fasttext, bert, or turl for using the respective embeddings.
vec_length = 300
...
while benchmark_selection not in (1, 2, 3):
    print("Which Benchmark? Press 1 for Align. Press 2 for Real. Press 3 for open_data_uk.")
...
elif benchmark_selection == 3:
    input_folder_name = r"open_data_uk/"

# For open_data_uk: tables live outside codes/, so we track the absolute path separately.
OPEN_DATA_UK_TABLES = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "..", "results", "alite", "open_data_uk", "clusters_for_alite"
)
OPEN_DATA_UK_EMBEDDINGS = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), method + "open_data_uk"
)

# Auto-generate BERT embeddings for open_data_uk if they don't exist yet.
if benchmark_selection == 3 and method == "bert/":
    if not glob.glob(os.path.join(OPEN_DATA_UK_EMBEDDINGS, "*.json")):
        generate_bert_embeddings_for_folder(OPEN_DATA_UK_TABLES, OPEN_DATA_UK_EMBEDDINGS)

foldername = method + input_folder_name
if benchmark_selection == 3:
    foldername = OPEN_DATA_UK_EMBEDDINGS + os.sep
...
for tablename in all_files:
    try:
        ...
        if benchmark_selection == 3:
            real_table_path = os.path.join(OPEN_DATA_UK_TABLES, cluster_name) + os.sep
        else:
