...
This folder packages [ALITE](https://www.vldb.org/pvldb/vol16/p932-khatiwada.pdf) (VLDB 2023) plus corpus scripts used as the Full Disjunction merge baseline in *Multi-Table Error Correction*.

**Start here:** [readme/corpus_pipeline.md](readme/corpus_pipeline.md) and `scripts/run_alite_corpus.sh`.
...
- **scripts/run_alite_corpus.sh** — one command to run the full ALITE corpus pipeline (index → discover → FD → backfill → plots).
- **readme/corpus_pipeline.md** — full start-to-finish guide for running ALITE on **open_data_uk** and **mit_dwh** (setup → BLEND index → cluster discovery → FD → error backfill → plots).
