...
def evaluate_cleaning_performance(clean_path, dirty_path, cleaned_path, elapsed_time, output_path,table_name,mse_attributes = []):
    ...
    # Add index column if not present in any of the dataframes
    if index_attribute not in clean_data.columns:
        clean_data[index_attribute] = range(len(clean_data))
    if index_attribute not in dirty_data.columns:
        dirty_data[index_attribute] = range(len(dirty_data))
    if index_attribute not in cleaned_data.columns:
        cleaned_data[index_attribute] = range(len(cleaned_data))
