...
# Gemini limits: 4000 RPM (requests per minute), 4M TPM (tokens per minute)
REQUEST_DELAY = 0.02  # 20ms delay = max 3000 requests/minute (safe margin below 4000)
last_request_time = 0
...
def rate_limit_sleep():
    """Sleep to enforce rate limiting between API requests"""
...
def initialize_llm(model_name, openai_api_base, openai_api_key, temperature):
    return ChatOpenAI(
        ...
        temperature = 1,
        max_tokens = 8192  # Increase max output tokens to prevent truncation
    )
    ...
    while True:
        try:
            ...
            if isinstance(repair_result, str):
                # Try to strip markdown and parse as JSON
                cleaned_result = strip_markdown_json(repair_result)
                try:
                    repair_result = json.loads(cleaned_result)
                except json.JSONDecodeError:
                    print(f'Warning: LLM returned string that could not be parsed as JSON, keeping original value')
                    try_num += 1
                    if try_num >= 3:  # Only retry 3 times for string responses
                        break
                    continue
            result_json = repair_result
            # Safely access correction dict
            if isinstance(result_json, dict) and 'correction' in result_json:
                if isinstance(result_json['correction'], dict):
                    if result_json['correction'].get(column) is None:
                        result_json['correction'][column] = 'null'
                    correction = result_json['correction'][column]
                else:
                    # If correction is not a dict, use it directly
                    correction = str(result_json['correction'])
            else:
                # If structure is wrong, keep original value
                print(f'Warning: Unexpected response structure, keeping original value')
                print(result_json)
                try_num += 1
                if try_num >= 3:
                    break
            ...
            # Try to extract and parse JSON from markdown-wrapped error messages
            if 'Invalid json output' in error_str or '```json' in error_str:
                # Try to extract JSON from the error message
                json_match = re.search(r'```json\s*\n?(.*?)\n?```', error_str, re.DOTALL)
                if json_match:
                    try:
                        cleaned_json = strip_markdown_json(json_match.group(0))
                        result_json = json.loads(cleaned_json)
                        # If we successfully parsed it, process the correction
                        if isinstance(result_json, dict) and 'correction' in result_json:
                            if isinstance(result_json['correction'], dict):
                                if result_json['correction'].get(column) is None:
                                    result_json['correction'][column] = 'null'
                            ...
                            print('Successfully recovered JSON from markdown-wrapped error')
                            break
                    except (json.JSONDecodeError, KeyError) as parse_error:
                        print(f'Could not recover JSON from error: {parse_error}')
            ...
            if '429' in error_str or 'RESOURCE_EXHAUSTED' in error_str:
                ...
                if retry_match:
                    wait_time = float(retry_match.group(1)) + 60  # Add 1 second buffer
                else:
                    wait_time = 60  # Default 60 seconds
                
                print(f'Rate limit hit. Waiting {wait_time:.1f} seconds before retry...')
                time.sleep(wait_time)
                continue  # Retry without incrementing try_num for rate limits
...
def calc_p_r_f(clean_data, dirty_data, corrected_data):
    ...
    Precision = right_corrected_num1 / corrected_num if corrected_num > 0 else 0
    ...
    Recall = right_corrected_num2 / dirty_num if dirty_num > 0 else 0
    F1 = harmonic_mean(Precision, Recall) if (Precision > 0 and Recall > 0) else 0
    metrics_output = f"""
{'='*60}
EVALUATION METRICS
{'='*60}
Total errors in dirty data: {dirty_num}
Total corrections attempted: {corrected_num}
Correct corrections: {right_corrected_num1}
Successfully fixed errors: {right_corrected_num2}

Precision: {Precision:.4f} ({right_corrected_num1}/{corrected_num})
Recall:    {Recall:.4f} ({right_corrected_num2}/{dirty_num})
F1-Score:  {F1:.4f}
{'='*60}
"""
    with open(os.path.join(output_path, 'output.txt'), 'a', encoding='utf-8') as f_output:
        f_output.write(metrics_output)
...
if __name__ == "__main__":
    ...
    total_tokens = 0
    human_repair_num = 0
    _zeroshot_dir = os.path.dirname(os.path.abspath(__file__))
    _base_zeroshot = os.path.dirname(_zeroshot_dir)
    dotenv.load_dotenv(dotenv_path=os.path.join(_base_zeroshot, '.env'), override=True)
    openai_api_key = os.getenv("OPENAI_API_KEY")
    google_api_key = os.getenv("GOOGLE_GENAI_API_KEY")
    parser = argparse.ArgumentParser(description='ZeroEC Zero-Shot Correction Script')
    parser.add_argument('--clean_data_path', type=str, default=None, help='Path to clean.csv')
    parser.add_argument('--dirty_data_path', type=str, default=None, help='Path to dirty.csv')
    parser.add_argument('--detection_path', type=str, default=None, help='Path to perfect_error_detection.csv')
    parser.add_argument('--output_path', type=str, default=None, help='Output directory')
    parser.add_argument('--dataset_name', type=str, default='beers', help='Dataset name')
    args = parser.parse_args()
    dataset_name = args.dataset_name
    _lake_bench = os.path.dirname(os.path.dirname(_base_zeroshot))  # LakeCorrectionBench
    _default_data = os.path.join(_lake_bench, 'datasets', 'address_pr_dataset_processed_without_city', 'isolated', 'table_a')
    clean_data_path = args.clean_data_path or os.path.join(_default_data, 'clean.csv')
    dirty_data_path = args.dirty_data_path or os.path.join(_default_data, 'dirty.csv')
    detection_path = args.detection_path or os.path.join(_default_data, 'perfect_error_detection.csv')
    output_path = args.output_path or os.path.join(_base_zeroshot, 'results', 'address_pr_dataset_processed_without_city', 'isolated', 'table_a')
    os.makedirs(output_path, exist_ok=True)
    # Create time cost file in output directory
    f_time_cost = open(os.path.join(output_path, 'time_cost.txt'), 'w', encoding='utf-8')
    # param-prompts_path (relative to this script)
    system_message_EC_path = os.path.join(_zeroshot_dir, 'prompt_templates', 'SystemMessage-2.txt')
    human_message_EC_path = os.path.join(_zeroshot_dir, 'prompt_templates', 'HumanMessage.txt')
    general_examples_EC_path = os.path.join(_zeroshot_dir, 'prompt_templates', 'examples.txt')
    ...
    # param-llm4EC
    # Use Gemini for zero-shot correction (same as ZeroEC/correction.py)
    USE_GEMINI_EC = True  # Set to False to use OpenAI (gpt-4o) instead
    TEMPERATURE_EC = 0
    if USE_GEMINI_EC and google_api_key:
        MODEL_NAME_EC = 'gemini-2.5-flash-lite'
        OPENAI_API_BASE_EC = 'https://generativelanguage.googleapis.com/v1beta/openai/'
        OPENAI_API_KEY_EC = google_api_key
    else:
        MODEL_NAME_EC = 'gpt-4o'
        OPENAI_API_BASE_EC = 'https://api.openai.com/v1/'
        OPENAI_API_KEY_EC = openai_api_key
    ...
    dirty_data = pd.read_csv(dirty_data_path, dtype=str, encoding='utf-8', keep_default_na=False)
    ...
    with open(os.path.join(output_path, 'output.txt'), 'a', encoding='utf-8') as f_output:
        f_output.write(f"\n{'='*60}\n")
        f_output.write(f"EXECUTION SUMMARY\n")
        f_output.write(f"{'='*60}\n")
        f_output.write(f"Total execution time: {execution_time:.2f} seconds\n")
        f_output.write(f"Total tokens used: {total_tokens}\n")
        f_output.write(f"Human repair samples: {human_repair_num}\n")
        f_output.write(f"{'='*60}\n")
    # Write final time summary
    f_time_cost.write(f"\n{'='*60}\n")
    f_time_cost.write(f"TOTAL EXECUTION TIME\n")
    f_time_cost.write(f"{'='*60}\n")
    f_time_cost.write(f"Total execution time: {execution_time:.2f} seconds\n")
    f_time_cost.close()