...
import pandas as pd
from holoclean.dataset.dataset import AuxTables
from holoclean.dataset.table import Source, Table
from holoclean.utils import NULL_REPR
from utils import read_csv, value_normalizer
...
class EvalEngine:
    ...
    def load_data(self, name, fpath, tid_col, attr_col, val_col, na_values=None):
        tic = time.clock()
        try:
            raw_data = pd.read_csv(fpath, sep=",", header="infer", low_memory=False, encoding="latin-1", dtype={"correct_val": str}, keep_default_na=False).applymap(lambda x: value_normalizer(x) if isinstance(x, str) else x)
    ...
    def evaluate_repairs(self):
        ...
        if self.env['verbose']:
            try:
                ...
                logging.error("ERROR logging weak label stats %s" % e)
    ...
    def eval_report(self):
        ...
        try:
            ...
            eval_report = EvalReport(prec, rec, rep_recall, f1, rep_f1, self.detected_errors, self.detected_errors,
