...
def run_metanome_with_cli(file_path):
    ...
    if system == "Windows":
        ...
        _metanome_root = Path(__file__).resolve().parents[2]
        _cp = f"{_metanome_root / 'metanome-cli-1.2-SNAPSHOT.jar'}:{_metanome_root / 'HyFD-1.2-SNAPSHOT.jar'}"
        val = subprocess.check_call(
            f"java -cp \"{_cp}\" de.metanome.cli.App --algorithm de.metanome.algorithms.hyfd.HyFD --files \"{file_path}\" --file-key \"INPUT_GENERATOR\" --header --separator \",\" -o \"file:clean\"",
