...
class Correction:
    ...
    def __init__(self, labeling_budget=20):
        """
        The constructor.
        """
    ...
    def initialize_dataset(self, d):
        ...
        d.labeled_tuples_selected_columns = {} if not hasattr(d, "labeled_tuples_selected_columns") else d.labeled_tuples_selected_columns    
    ...
    def label_with_ground_truth(self, d, column_budgets):
        """
        This method labels a tuple with ground truth.
        """
        
        d.labeled_tuples_selected_columns[d.sampled_tuple] = []
        for j in range(d.dataframe.shape[1]):
            if j in column_budgets:
                if column_budgets[j] > 0:
                    ...
                    d.labeled_tuples_selected_columns[d.sampled_tuple].append(j)
    ...
    def update_models(self, d):
        ...
        """
        cleaned_sampled_tuple = {j: d.labeled_cells[(d.sampled_tuple, j)][1] for j in d.labeled_tuples_selected_columns[d.sampled_tuple]}
        for j in d.labeled_tuples_selected_columns[d.sampled_tuple]:
            cell = (d.sampled_tuple, j)
            update_dictionary = {
                "column": cell[1],
            ...
            if d.labeled_cells[cell][0] == 1:
                ...
                update_dictionary["vicinity"] = [cv if j != cj else self.IGNORE_SIGN
                                                 for cj, cv in cleaned_sampled_tuple.items()]
            else:
                update_dictionary["vicinity"] = [cv if j != cj and d.labeled_cells[(d.sampled_tuple, cj)][0] == 1
                                                 else self.IGNORE_SIGN for cj, cv in cleaned_sampled_tuple.items()]
            self._vicinity_based_models_updater(d.vicinity_models, update_dictionary)
        if self.VERBOSE:
    ...
    def run(self, d, column_budgets):
        ...
        if self.VERBOSE:
            print("------------------------------------------------------------------------\n"
                  ...
                  "------------------------------------------------------------------------")
        rep_iter = 0
        old_sampled_tuple = None
        while len(d.labeled_cells) < sum(column_budgets.values()) and len(d.labeled_tuples) < len(d.dataframe):
            if self.VERBOSE:
                print(f"Label round {len(d.labeled_cells)}/{self.LABELING_BUDGET}")
            self.sample_tuple(d)
            if old_sampled_tuple == d.sampled_tuple:
                rep_iter += 1
            else:
                rep_iter = 0
            if rep_iter > 5:
            ...
            if d.has_ground_truth:
                self.label_with_ground_truth(d, column_budgets)
            # else:
            ...
            if self.VERBOSE:
                print("------------------------------------------------------------------------")
            old_sampled_tuple = copy.deepcopy(d.sampled_tuple)
        # self.predict_corrections(d)
...
def main(results_path, dataset_path, dataset_name, labeling_budget, execution_number, column_budgets):
    ...
    correction_dictionary, labeled_cells = app.run(data, column_budgets)
    end_time = time.time()
    ...
    results = {'dataset_path': dataset_path, 'dataset_name': dataset_name, 'execution_number': execution_number, 'dataset_shape': data.dataframe.shape, 
               ...
               'execution-time': end_time - start_time, 'number_of_labeled_tuples': len(data.labeled_tuples),
    ...
    res_path_base = os.path.join(results_path, f'''raha_{dataset_name}_number#{execution_number}_${labeling_budget}''')
