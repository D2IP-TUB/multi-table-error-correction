...
class Correction:
    ...
    def label_with_ground_truth(self, d):
        ...
        for cell in d.sampled_tuple_cells:
            error_label = 0
    ...
    def update_models(self, d):
        ...
        """
        # Build dictionary of labeled cell values indexed by column
        cleaned_tuple_dict = {j: d.labeled_cells[(d.sampled_tuple, j)][1] 
                              for j in range(d.dataframe.shape[1]) 
                              if (d.sampled_tuple, j) in d.labeled_cells}
        
        for j in range(d.dataframe.shape[1]):
            if (d.sampled_tuple, j) not in d.labeled_cells:
                continue
            cell = (d.sampled_tuple, j)
            update_dictionary = {
                "column": cell[1],
                "old_value": d.dataframe.iloc[cell],
                "new_value": cleaned_tuple_dict[j],
            }
            if d.labeled_cells[cell][0] == 1:
                ...
                self._domain_based_model_updater(d.domain_models, update_dictionary)
                # Build vicinity using labeled values where available, original values otherwise
                vicinity = [cleaned_tuple_dict.get(cj, d.dataframe.iloc[(d.sampled_tuple, cj)]) 
                           if j != cj else self.IGNORE_SIGN
                           for cj in range(d.dataframe.shape[1])]
                update_dictionary["vicinity"] = vicinity
            else:
                # For non-error cells, only use values from other labeled error cells
                vicinity = [cleaned_tuple_dict.get(cj, d.dataframe.iloc[(d.sampled_tuple, cj)]) 
                           if j != cj and (d.sampled_tuple, cj) in d.labeled_cells and d.labeled_cells[(d.sampled_tuple, cj)][0] == 1
        ...
        for column_idx, j in enumerate(d.column_errors):
            ...
            if x_train:
                ...
                if self.CLASSIFICATION_MODEL == "ABC":
                    classification_model = sklearn.ensemble.AdaBoostClassifier(n_estimators=100, random_state=None)
    ...
    def run(self, d):
        ...
        if self.VERBOSE:
            print("------------------------------------------------------------------------\n"
                  ...
                  "------------------------------------------------------------------------")
        while len(d.samples_predefined) > 0:
            if self.VERBOSE:
                print(f"Label round {len(d.labeled_tuples)+1}/{self.LABELING_BUDGET}")
            d.sampled_tuple_cells = d.samples_predefined.pop(0)
            d.sampled_tuple = d.sampled_tuple_cells[0][0] if d.sampled_tuple_cells else None
        ...
        return d.corrected_cells, d.labeled_cells
########################################
...
def main(results_path, dataset_path, dataset_name, labeling_budget, execution_number, column_wise_evaluation, column_idx, sampled_tuples):
    ...
    data = raha.dataset.Dataset(dataset_dictionary)
    data.detected_cells = dict(data.get_actual_errors_dictionary())
    data.samples_predefined = []
    for sample_tuple in sampled_tuples:
        tuple_samples = []
        for cell in sample_tuple:
            col_idx = data.dataframe.columns.get_loc(cell[1])
            tuple_samples.append((cell[0], col_idx))
        data.samples_predefined.append(tuple_samples)
    app = Correction(labeling_budget)
    correction_dictionary, labeled_cells = app.run(data)
    end_time = time.time()
    data = raha.dataset.Dataset(dataset_dictionary)
    corrected_errors = list(correction_dictionary.keys())
    if column_wise_evaluation:
        metrics = data.get_data_cleaning_evaluation_selected_col(correction_dictionary, sampled_rows_dictionary=False, col_idx=column_idx)
    else:
        metrics = data.get_data_cleaning_evaluation(correction_dictionary)
    err_labeled_cells = [(dataset_name, cell[0], cell[1]) for cell in labeled_cells if labeled_cells[cell][0]==1]
    results = {'dataset_path': dataset_path, 'dataset_name': dataset_name, 'execution_number': execution_number, 'dataset_shape': data.dataframe.shape, 
               'precision': metrics["ec_p"], 'recall': metrics["ec_r"], 'f_score': metrics["ec_f"],
               'tp': metrics["ec_tp"], 'ec_tpfp': metrics["output_size"], 'ec_tpfn': metrics["actual_errors"],
               'execution-time': end_time - start_time, 'number_of_labeled_tuples': labeling_budget,
               'number_of_labeled_cells': len(err_labeled_cells), 'corrected_errors_keys': corrected_errors,
               'true_postives_cells': metrics["true_postives_cells"],
               'false_positives_cells': metrics.get("false_positives_cells", {}),
               }
    ...
    res_path_base = os.path.join(results_path, f'''raha_{dataset_name}_col_{column_idx}_number#{execution_number}_${labeling_budget}''')
    with open(res_path_base + "labels_samples.pickle", 'wb') as act:
        pickle.dump(err_labeled_cells, act)
    if not os.path.exists(act_errors_dirs):
        os.makedirs(act_errors_dirs)
    with open(result_file_path, "w") as result_file:
        json.dump(results, result_file)
    # with open(act_errors_path, 'wb') as act:
    #     pickle.dump(data.detected_cells, act)
